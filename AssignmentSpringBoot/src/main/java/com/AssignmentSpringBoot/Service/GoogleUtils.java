...
public class GoogleUtils {
	...
	public UserDetails builUser(GooglePojo googlePojo) {
		...
		List<com.AssignmentSpringBoot.Entity.User> users = userRepository.findByEmail(googlePojo.getEmail());
		if (users == null || users.isEmpty()) {
			com.AssignmentSpringBoot.Entity.User newUser = new com.AssignmentSpringBoot.Entity.User();
			...
			com.AssignmentSpringBoot.Entity.User user = users.get(0);
			List<GrantedAuthority> authorities = new ArrayList<GrantedAuthority>();
			authorities.add(new SimpleGrantedAuthority("ROLE_USER"));
			UserDetails userDetail = new User(user.getUsername(), user.getPassword(), enabled, accountNonExpired, credentialsNonExpired,
